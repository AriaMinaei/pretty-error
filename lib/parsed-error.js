...
module.exports = ParsedError = (function() {
  ...
  ParsedError.prototype._parse = function() {
    ...
    if (typeof this.error !== 'object') {
      ...
      this._stack = (this.error.stack != null) && this.error.stack || null;
  ...
  ParsedError.prototype._parseTrace = function() {
    if (typeof this._stack === 'string') {
      this._parseTextAsStack(this._stack);
    } else if (Array.isArray(this._stack)) {
      this._parseArrayAsStack(this._stack);
    }
  };

  ParsedError.prototype._parseArrayAsStack = function(a) {
    var item, _i, _len;
    for (_i = 0, _len = a.length; _i < _len; _i++) {
      item = a[_i];
      this._trace.push(String(item));
    }
  };

  ParsedError.prototype._parseTextAsStack = function(text) {
    var line, _i, _len, _ref;
    text = text.replace(/^([a-zA-Z0-9\_\$]+):\ /, '');
    if (text.substr(0, this.message.length) === this.message) {
      text = text.substr(this.message.length, text.length);
    }
    text = text.trim();
    _ref = text.split("\n");
